  ...
  <div class="min-h-screen bg-slate-950 flex items-center justify-center px-3">
    ...
    >
      <div v-if="state === 'loading'" class="space-y-2">
        <h1 class="text-xl font-semibold text-slate-50">Verifying your email...</h1>
        <p class="cms-caption">Please wait a moment.</p>
      </div>

      <div v-else-if="state === 'success'" class="space-y-3">
        <h1 class="text-xl font-semibold text-emerald-400">Email verified</h1>
        <p class="cms-caption">Your email has been successfully verified. You can now sign in.</p>
      ...
      <div v-else class="space-y-3">
        <h1 class="text-xl font-semibold text-rose-400">Verification failed</h1>
        <p class="cms-caption">The verification link is invalid or has expired.</p>
