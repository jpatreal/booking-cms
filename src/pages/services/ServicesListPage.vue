<template>
  <div class="space-y-5">
    ...
    >
      ...
      <div class="flex items-center gap-3 cms-helper text-slate-400">
        ...
        <select
          ...
          class="bg-slate-950 border border-slate-800 rounded-xl text-sm px-2.5 py-1.5 text-slate-300 outline-none"
        >
    ...
    <div class="bg-slate-900/80 border border-slate-800 rounded-2xl">
      <!-- header row -->
      <div
        class="grid grid-cols-[minmax(0,3fr)_minmax(0,1fr)_minmax(0,1fr)_minmax(0,0.8fr)_minmax(0,0.8fr)] gap-3 px-4 py-2 text-xs text-slate-500 border-b border-slate-800 uppercase tracking-wide"
      >
        <div>Service</div>
        <div>Price</div>
        <div>Duration</div>
      ...
      <!-- loading / empty -->
      <div v-if="loading" class="px-4 py-6 cms-helper">Loading services...</div>

      <div v-else-if="services.length === 0" class="px-4 py-6 cms-caption">
      ...
      <div v-else class="divide-y divide-slate-800">
        <div
          v-for="s in services"
          :key="s.id"
          class="grid grid-cols-[minmax(0,3fr)_minmax(0,1fr)_minmax(0,1fr)_minmax(0,0.8fr)_minmax(0,0.8fr)] gap-3 px-4 py-3 text-sm items-center hover:bg-slate-900"
        >
          <!-- Service info -->
          <div class="flex flex-col">
            ...
            <div v-if="s.description" class="cms-helper text-slate-500 line-clamp-1">
              {{ s.description }}
            </div>
          </div>
    ...
    >
      <form class="space-y-3" @submit.prevent="save">
        ...
        <label class="flex items-center gap-2 cms-helper text-slate-300">
          <input type="checkbox" v-model="form.active" class="w-4 h-4 accent-brand-500" />
